...
class TestOpenAIProvider:
    ...
    def test_model_validation(self):
        ...
        assert provider.validate_model_name("gpt-5.3-codex-spark") is True
        ...
        # retired-model aliases still resolve onto their live replacements
    ...
    def test_resolve_model_name(self):
        ...
        assert provider._resolve_model_name("gpt5.1-codex") == "gpt-5.3-codex"
        assert provider._resolve_model_name("codex-mini") == "gpt-5.3-codex-spark"
        ...
        assert provider._resolve_model_name("gpt-5.3-codex-spark") == "gpt-5.3-codex-spark"
    ...
    def test_get_capabilities_gpt53_codex_spark(self):
        """Test GPT-5.3 Codex Spark is the cost-efficient Codex lane."""
        provider = OpenAIModelProvider("test-key")

        capabilities = provider.get_capabilities("gpt-5.3-codex-spark")
        assert capabilities.model_name == "gpt-5.3-codex-spark"
    ...
    async def test_generate_content_other_aliases(self, mock_openai_class):
        ...
        await provider.generate_content(prompt="Test", model_name="o3mini", temperature=1.0)
        call_kwargs = mock_client.chat.completions.create.call_args[1]
        assert call_kwargs["model"] == "o3-mini"

        # Test o4mini -> o4-mini
        mock_response.model = "o4-mini"
        await provider.generate_content(prompt="Test", model_name="o4mini", temperature=1.0)
        call_kwargs = mock_client.chat.completions.create.call_args[1]
    ...
    async def test_generate_content_no_alias_passthrough(self, mock_openai_class):
        ...
        await provider.generate_content(prompt="Test", model_name="o3-mini", temperature=1.0)
        call_kwargs = mock_client.chat.completions.create.call_args[1]
    ...
    async def test_o3_pro_routes_to_responses_endpoint(self, mock_openai_class):
        ...
        result = await provider.generate_content(prompt="What is 2 + 2?", model_name="o3-pro", temperature=1.0)
    ...
    async def test_non_o3_pro_uses_chat_completions(self, mock_openai_class):
        ...
        result = await provider.generate_content(prompt="Test prompt", model_name="o3-mini", temperature=1.0)
