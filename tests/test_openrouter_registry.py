...
class TestOpenRouterModelRegistry:
    ...
    def test_alias_resolution(self):
        ...
        test_cases = [
            ("opus", "anthropic/claude-opus-5"),  # opus now points to Opus 5
            ("OPUS", "anthropic/claude-opus-5"),  # Case insensitive
    ...
    def test_model_capabilities_conversion(self):
        ...
        # opus alias now points to Opus 5
        assert config.provider == ProviderType.OPENROUTER
        assert config.model_name == "anthropic/claude-opus-5"
        assert config.friendly_name == "OpenRouter (anthropic/claude-opus-5)"
