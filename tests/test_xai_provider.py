...
class TestXAIProvider:
    ...
    def test_model_validation(self):
        ...
        # Test valid models
        assert provider.validate_model_name("grok-4.7") is True
    ...
    def test_resolve_model_name(self):
        ...
        assert provider._resolve_model_name("grok-4.7") == "grok-4.7"
        assert provider._resolve_model_name("grok-4.1-fast") == "grok-4.3"

    def test_get_capabilities_grok47(self):
        """Test getting model capabilities for GROK-4.6."""
        provider = XAIModelProvider("test-key")

        capabilities = provider.get_capabilities("grok-4.7")
    ...
    @patch.dict(os.environ, {"XAI_ALLOWED_MODELS": "grok-4.7"})
    def test_model_restrictions(self):
        ...
        # grok-4.7 should be allowed (including alias)
        assert provider.validate_model_name("grok-4.7") is True
        assert provider.validate_model_name("grok") is True
    ...
    def test_multiple_model_restrictions(self):
        ...
        # grok-4.7 should NOT be allowed
        assert provider.validate_model_name("grok-4.7") is False

    @patch.dict(os.environ, {"XAI_ALLOWED_MODELS": "grok,grok-4.7,grok-4.1-fast,grok-4.3"})
    def test_both_shorthand_and_full_name_allowed(self):
        ...
        assert provider.validate_model_name("grok-4.7") is True  # Canonical name explicitly allowed
    ...
    def test_supported_models_structure(self):
        ...
        assert isinstance(grok47_config, ModelCapabilities)
        assert hasattr(grok47_config, "context_window")
        assert hasattr(grok47_config, "supports_extended_thinking")
        assert hasattr(grok47_config, "aliases")
        ...
        assert "grok-4.7" in grok47_config.aliases
    ...
    async def test_generate_content_resolves_alias_before_api_call(self, mock_openai_class):
        ...
        to 'grok-4.7' before being sent to X.AI API.
        """
    ...
    async def test_generate_content_other_aliases(self, mock_openai_class):
        ...
        # Test grok-4.7 -> grok-4.7
        await provider.generate_content(prompt="Test", model_name="grok-4.7", temperature=0.7)
        call_kwargs = mock_client.chat.completions.create.call_args[1]
        assert call_kwargs["model"] == "grok-4.7"
